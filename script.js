...
function updateCart() {
    ...
    const cartHTML = cart.map((item, index) => {
        total += item.qty * 8000;
        return `<div class="bg-white p-2 border border-[#3d1c02] rounded-xl text-xs flex justify-between items-center"><div class="flex items-center gap-3"><img src="${item.f}" class="w-10 h-10 object-cover rounded-lg border"><div><b class="uppercase">${item.qty}x ${item.n}</b><br><span class="uppercase text-[9px]">${item.type}</span></div></div><button onclick="cart.splice(${index},1); updateCart();" class="text-red-500 font-bold text-xl ml-2">&times;</button></div>`;
    }).join('');
