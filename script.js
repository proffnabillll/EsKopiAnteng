    ...
    const html = cart.map((i, idx) => {
        ...
        <div class="bg-white p-2 border border-[#3d1c02] rounded-xl text-xs flex justify-between items-center animate-pop mb-2">
            <div class="flex items-center gap-3">
                ...
                <div><b class="uppercase">${i.qty}x ${i.n}</b><br><span class="uppercase text-[9px]">${i.type}</span></div>
            </div>
            <button onclick="cart.splice(${idx},1); updateCart();" class="text-red-500 font-bold text-xl ml-2">&times;</button>
        </div>`;
    }).join('');

    if(listDesk) listDesk.innerHTML = cart.length ? html : '<p class="text-center text-gray-400 mt-10 text-xs italic">Kosong</p>';
    if(listMob) listMob.innerHTML = cart.length ? html : '<p class="text-center text-gray-400 py-10 italic text-xs">Kosong</p>';
...
function openModal(id) { activeItem = products.find(p => p.id === id); document.getElementById('m-name').innerText = activeItem.n; currentQty = 1; document.getElementById('m-qty').innerText = currentQty; selectedType = "ICE"; document.getElementById('btn-ice').style.borderColor = '#3d1c02'; document.getElementById('btn-hot').style.borderColor = 'transparent'; document.getElementById('modal-icehot').style.display = 'flex'; }
