        ...
        container.innerHTML += `
            <div class="bg-white p-3 rounded-2xl border-2 border-[#3d1c02] shadow-sm flex flex-col animate-pop relative" style="animation-delay: ${idx*0.05}s">
                <div class="photo-box cursor-pointer" onclick="openModal(${p.id})">
                    <img src="${p.f || 'https://via.placeholder.com/300x400?text=ANTENG+KOPI'}">
                </div>
                <h3 class="font-bold text-xs mt-2 uppercase truncate text-center">${p.n}</h3>
                <div class="flex justify-center items-center mt-auto pt-2 min-h-[40px]">
                    <span class="text-amber-700 font-black text-sm text-center">Rp. 8000</span>
                    <button onclick="playSound('click'); openModal(${p.id})" class="bg-[#3d1c02] text-white w-9 h-9 rounded-xl font-bold btn-bounce text-xl absolute right-3 bottom-3">+</button>
                </div>
            </div>`;
    });
}
...
function openMobileCart() { 
    document.getElementById('modal-mobile-cart').style.display = 'flex'; 
}

function closeMobileCart() { 
    document.getElementById('modal-mobile-cart').style.display = 'none'; 
}
...
function openPay() { 
    ...
    if(!name.trim()) return alert("Isi nama pelanggan!");
...
function finalize(withPrint) {
    if (withPrint) {
        ...
        } else document.getElementById('p-split').innerHTML = "";
