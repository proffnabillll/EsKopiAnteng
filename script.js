...
function updateCart() {
    ...
    const html = cart.map((i, idx) => {
        total += i.qty * 8000;
        return `<div class="bg-white p-2 border border-[#3d1c02] rounded-xl text-xs flex justify-between items-center animate-pop"><div class="flex items-center gap-3"><img src="${i.f}" class="w-10 h-10 object-cover rounded-lg border"><div><b class="uppercase">${i.qty}x ${i.n}</b><br><span class="uppercase text-[9px]">${i.type}</span></div></div><button onclick="cart.splice(${idx},1); updateCart();" class="text-red-500 font-bold text-xl ml-2">&times;</button></div>`;
    }).join('');
    listDesk.innerHTML = cart.length ? html : '<p class="text-center text-gray-400 mt-10 text-xs italic">Kosong</p>';
    listMob.innerHTML = cart.length ? html : '<p class="text-center text-gray-400 py-10 italic text-xs">Kosong</p>';
...
function openPay() { 
    ...
    if(cart.length === 0 || !nameInput.value.trim()) return alert("Lengkapi pesanan & nama!");
